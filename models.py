...
def All_Matern_Model(X, y, use_priors = False, e_s = 0, period = 12 ):
    ...
    with gpflow.defer_build():
        ...
        like = gpflow.likelihoods.Poisson(binsize = e_s)
        kern_s_effect = gpflow.kernels.Matern52(input_dim =  2, active_dims=[1,2], name='space_effect')
        kern_t_effect = gpflow.kernels.Matern32(1, active_dims=[0], name='time_effect')
        ...
        if use_priors:
                #m.kern.matern32.variance.prior    = t_prior
                m.kern.periodic.variance.prior = t_prior
                #m.kern.matern52.variance.prior    = t_prior

                #m.kern.matern32.lengthscales.prior = t_prior
...
def Poisson_Model_T(X, y, use_priors = False, e_s = 0, period = 12):
    ...
    with gpflow.defer_build():
            ...
            t_prior = gpflow.priors.StudentT(mean = 0 , scale = 1, deg_free = 10)
